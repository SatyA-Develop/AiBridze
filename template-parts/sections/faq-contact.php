	...
	<div class="faq-contact__inner">
		...
		<div class="faq-contact__form-panel">
			...
			<form class="faq-contact__form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
				...
				<label><span class="screen-reader-text"><?php echo esc_html( $args['budget_label'] . ' (' . __( 'Optional', 'aibridze' ) . ')' ); ?></span><select name="budget"><option value=""><?php echo esc_html( str_replace( 'Your Preferred Budget Range', 'Preferred Budget', $args['budget_label'] ) . ' (' . __( 'Optional', 'aibridze' ) . ')' ); ?></option><?php foreach ( array_filter( array_map( 'trim', (array) $args['budget_options'] ) ) as $option ) : ?><option><?php echo esc_html( $option ); ?></option><?php endforeach; ?></select></label>
